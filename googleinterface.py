...
def get_endocap_values():
    values = [
        get_timestamp("%Y/%m/%d"),
        str(data["endo_cap"]),
        str(data["endo_caution_line"]),
        str(config["data"]["endo_caution_line_perc"]),
...
def get_delegate_transition_values():
    values = [
        ...
        str(data["max_in_endo_num"]),
        str(
            get_value_from_list(data["in_endo_sorted_list"], config["data"]["delegate"])
        ),
    ]
...
def append_value_sheet(service, sheet_id, range, values):
    ...
    service.values().append(
        spreadsheetId=sheet_id, range=range, body=body, valueInputOption="USER_ENTERED"
    ).execute()
...
def update_sheet():
    ...
    append_value_sheet(
        service, sheet_config["general"], range_config["stats"], stats_values
    )
    logger.info("Updated stats sheet")

    endocap_values = get_endocap_values()
    append_value_sheet(
        service, sheet_config["general"], range_config["endo_cap"], endocap_values
    )

    SPCG_endocap_values = get_SPCG_endocap_values()
    append_value_sheet(
        service,
        sheet_config["general"],
        range_config["SPCG_stats"],
        SPCG_endocap_values,
    )
    ...
    if config["delegate_transition"]:
        ...
        append_value_sheet(
            service,
            sheet_config["delegate_transition"],
