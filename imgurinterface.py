...
from utils import config, get_logger, get_timestamp, add_timestamp
...
def authenticate(imgur):
    ...
    try:
        ...
        logger.error('Failed to authenticate. Error %d',
...
def get_imgur():
    ...
    if refresh_token_file.read() == "":
        ...
        imgur = pyimgur.Imgur(imgur_auth_config['client_id'],
...
def upload_image():
    ...
    try:
        img = imgur.upload_image(path=config['final_image']['small_save_path'],
        ...
        logger.error('Failed to upload to Imgur. Error: %d',
                     e.response.status_code, exc_info=True)
