...
def calc_box_margin(base_image, image):
    ...
    logger.debug('Left margin: %f | Top margin: %f',
...
def composite(image):
    ...
    resolution = (img_config['resolution']['width'],
                  img_config['resolution']['height'])
...
def text_overlay(image):
    ...
    try:
        ...
        text_config['font_path'], text_config['font_size'])
    except IOError:
    ...
    ctx.multiline_text(xy=text_position,
                       text=text,
                       font=font,
                       fill=font_color,
...
def resize(image):
    resolution = (config['final_image']['small_resolution']['width'],
                  config['final_image']['small_resolution']['height'])
    image.thumbnail(size=resolution,
