...
def get_increment(minmax_config, max_val):
    config_range = float(minmax_config["max"]) - float(minmax_config["min"])
...
def mapping(gv_graph):
    ...
    node_size_increment = get_increment(node_config["size"], data["max_in_endo_num"])
    logger.debug("Node size increment: %f", node_size_increment)

    node_hue_increment = get_increment(
        node_config["fill_color"]["hue"], data["max_out_endo_num"]
    )
    logger.debug("Node hue increment: %f", node_hue_increment)

    font_size_increment = get_increment(
        node_config["label"]["font_size"], data["max_in_endo_num"]
    )
    ...
    for node in gv_graph.nodes():
        node.attr["width"] = get_attr_val(
            node_config["size"], node_size_increment, gv_graph.in_degree(node)
        )

        node.attr["fontsize"] = get_attr_val(
            node_config["label"]["font_size"],
            font_size_increment,
        ...
        color_str = "{} {} {}".format(
            node_color_hue_str, node_color_saturation_str, node_color_value_str
        )
