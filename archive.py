...
def get_last_table_data(cursor, nation, column):
    ...
    logger.debug(
        'Got last table data of "%s", column "%s": %r', nation, column, last_data
    )
    return last_data
...
def get_data_last_row(cursor, table, column):
    ...
    logger.debug(
        'Got last data of table "%s", column "%s": %r', table, column, last_data
    )
    return last_data
...
def get_last_endo_data(cursor, nation):
    # Get last given endo
    last_given_endo = get_data_from_table(cursor, "accu", nation, "given_endo")[0]
    logger.debug('Got last given endo of "%s": %r', nation, last_given_endo)

    # Get last out endo
    last_out_endo_list = get_last_table_data(cursor, nation, "out_endo_list")
    logger.debug('Got last out endo of "%s": %r', nation, last_out_endo_list)
...
def get_total_value(cursor, nation, column, key):
    ...
    last_total_value = get_data_from_table(cursor, "accu", nation, column)[0]

    total_value = last_total_value + value

    logger.debug('Got total value "%s" of "%s": %r', key, nation, total_value)
    return total_value
...
def add_new_nation(cursor, nation):
    ...
    is_exist = check_if_exist(cursor, "accu", params["nation"])
    if is_exist is False:
        query_str_2 = """INSERT INTO "accu"
                      VALUES (:nation, 0, 0, 0)"""
        cursor.execute(query_str_2, params)
        logger.debug('Inserted new nation "%s" to accu table', params["nation"])
...
def update_accu_table(cursor, nx_graph, nation):
    ...
    total_gained_influence = get_total_value(
        cursor, nation, "gained_influence", "gained_influence_dict"
    )
    total_generated_influence = get_total_value(
        cursor, nation, "generated_influence", "generated_influence_dict"
    )
...
def update_in_out_endo_table(cursor, gen_time, nx_graph, nation):
    params = {
        "nation": nation,
        "out_endo": nx_graph.out_degree(nation),
...
def create_table_index(cursor, name, index, idx_columns):
    if index is not None:
        ...
        logger.debug(
            'Created table index "%s" on table "%s", columns "%s"',
            index,
            name,
            idx_columns,
        )
...
def save_to_archive():
    ...
    award_cursor, award_conn = get_sql_interface(
        archive_config["award_db_path"], backup=True
    )
    stats_cursor, stats_conn = get_sql_interface(
        archive_config["stats_db_path"], backup=True
    )
...
def get_average(cursor, nation, column, tables):
    ...
    count = 0
    for table in tables:
        query_str = "SELECT {} FROM '{}' WHERE nation=:nation".format(column, table[0])
        params = {"nation": nation}
        result = cursor.execute(query_str, params).fetchone()
...
def get_in_out_endo_rank(cursor, nations, tables):
    ...
    for nation in nations:
        if nation in config["data"]["excluded"] or nation not in data["wa_nations"]:
            continue
        avg_in_endos[nation] = get_average(cursor, nation, "in_endo", tables)
        avg_out_endos[nation] = get_average(cursor, nation, "out_endo", tables)

    in_endo_rank = [
        i[0]
        for i in sorted(avg_in_endos.items(), key=operator.itemgetter(1), reverse=True)
    ]
    out_endo_rank = [
        i[0]
        for i in sorted(avg_out_endos.items(), key=operator.itemgetter(1), reverse=True)
    ]
...
def get_rank_of_unique_tables(cursor):
    ...
    given_endo_rank = [
        i[0]
        for i in results
        if i[0] not in config["data"]["excluded"] and i[0] in data["wa_nations"]
    ]
...
def get_in_endo_census(cursor):
    ...
    for i in data["in_endo_sorted_list"]:
        nation = i[0]
        in_endo_num = i[1]
        change = get_census_change(cursor, nation, "in_endo", "in_endo_sorted_list")
        gained_influence = get_data_from_table(
            cursor, "accu", nation, "gained_influence"
        )[0]
...
def get_out_endo_census(cursor):
    ...
    for i in data["out_endo_sorted_list"]:
        ...
        change = get_census_change(cursor, nation, "out_endo", "out_endo_sorted_list")
        perc_wa_nation = float(out_endo_num) / float(data["wa_nation_num"] - 1) * 100
        generated_influence = get_data_from_table(
            cursor, "accu", nation, "generated_influence"
        )[0]

        census.append(
            [nation, out_endo_num, change, perc_wa_nation, generated_influence]
        )
...
def get_new_wa_nations(cursor):
    ...
    last_wa_nation_set = set(
        get_data_last_row(cursor, "stats", "wa_nations")[0].split(",")
    )
...
def read_from_archive():
    ...
    data["wa_nation_num_change"] = get_change_from_stats(
        stats_cursor, "wa_nation_num", "wa_nation_num"
    )
    data["perc_wa_regional_nation_num_change"] = get_change_from_stats(
        stats_cursor, "perc_wa_regional_nation_num", "perc_wa_to_region"
    )
    data["perc_wa_ns_wa_change"] = get_change_from_stats(
        stats_cursor, "perc_wa_ns_wa", "perc_wa_to_ns_wa"
    )

    data["regional_nation_num_change"] = get_change_from_stats(
        stats_cursor, "regional_nation_num", "regional_nation_num"
    )
    data["perc_regional_nation_num_ns_change"] = get_change_from_stats(
        stats_cursor, "perc_regional_nation_num_ns", "perc_regional_nation_num_ns"
    )

    data["max_in_endo_num_change"] = data["in_endo_census"][0][2]
    data["perc_max_in_endo_num_wa_change"] = get_change_from_stats(
        stats_cursor, "perc_max_in_endo_num_wa", "perc_max_in_endo_num_wa"
    )

    data["SPCG_avg_in_endo_num_change"] = get_change_from_stats(
        stats_cursor, "SPCG_avg_in_endo_num", "SPCG_avg_in_endo_num"
    )

    data["crsdel_num_change"] = get_change_from_stats(
        stats_cursor, "crsdel_num", "crsdel_num"
    )
    data["perc_crsdel_num_wa_change"] = get_change_from_stats(
        stats_cursor, "perc_crsdel_num_wa", "perc_crsdel_num_wa"
    )

    data["endo_num_change"] = get_change_from_stats(
        stats_cursor, "endo_num", "endo_num"
    )
    data["density_num_change"] = get_change_from_stats(
        stats_cursor, "density_num", "density_num"
    )
